...
class Test(TestBase):
    ...
    def testTableWithSchema(self):
        ...
        self.odps_with_schema.delete_table(
            test_table_name, schema=TEST_CLS_SCHEMA_NAME, if_exists=True
        )

        table = self.odps_with_schema.create_table(
            test_table_name,
            ("col1 string", "pt string"),
            schema=TEST_CLS_SCHEMA_NAME,
            lifecycle=1,
        )
        self.assertEqual(table.get_schema().name, TEST_CLS_SCHEMA_NAME)

        tables = list(
            self.odps_with_schema.list_tables(prefix=test_table_name, schema=TEST_CLS_SCHEMA_NAME)
        )
        assert len(tables) == 1
        assert tables[0].name == test_table_name
        assert tables[0].get_schema().name == TEST_CLS_SCHEMA_NAME
        ...
        table.drop()
        assert not self.odps_with_schema.exist_table(test_table_name, schema=TEST_CLS_SCHEMA_NAME)
