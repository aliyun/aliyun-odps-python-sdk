...
try:
    from odps.tunnel.pdio import TunnelPandasReader, TunnelPandasWriter
except ImportError:
...
class Test(TestBase):
    def setUp(self):
        ...
        self._old_pd_mem_cache_size = options.tunnel.pd_mem_cache_size

    def tearDown(self):
        super(Test, self).tearDown()
        options.tunnel.pd_mem_cache_size = self._old_pd_mem_cache_size

    @unittest.skipIf(not TunnelPandasReader, 'Accelerated pandas IO not available')
    ...
    @unittest.skipIf(not pd, 'pandas not available')
    def testRead(self):
        options.tunnel.pd_mem_cache_size = 200
    ...
    @unittest.skipIf(not TunnelPandasWriter, 'Accelerated pandas IO not available')
