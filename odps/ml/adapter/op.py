...
class DFAdapterOperation(object):
    def execute(self, sources, target):
        ...
        target._fields = copy.deepcopy(fields[0])

    @staticmethod
    def _get_fields_list_from_eps(eps):
        def _filter_fields(ep):
            fields = set(c.name for c in ep.df.schema.columns)
            return [f for f in ep._fields if f.name in fields]

        return copy.deepcopy([_filter_fields(ep) for ep in eps])
    ...
    def _set_singleton_role(fields, role_mapping):
        ...
        for f in fields:
            if f.name in role_mapping:
                yield MLField.copy(f, role_mapping[f.name])
            elif roles & f.role:
                ret_field = MLField.copy(f)
    ...
    def _add_field_roles(cls, fields, name_set, role, augment):
        ...
        for f in fields:
            if f.name in name_set:
                yield MLField.copy(f, role)
            elif role in f.role:
                if augment:
                    yield MLField.copy(f)
                else:
                    ret_field = MLField.copy(f)
...
class BatchRoleOperation(DFAdapterOperation):
    ...
    def execute(self, sources, target):
        ...
        target._fields = list(self._add_field_roles(fields[0], feature_set, self.role, self.augment))


class ExcludeFieldsOperation(DFAdapterOperation):
    ...
    def execute(self, sources, target):
        ...
        target._fields = list(self._clear_field_roles(fields[0], feature_set))


class SingletonRoleOperation(DFAdapterOperation):
    ...
    def execute(self, sources, target):
        ...
        if self.clear_feature:
            ret_fields = list(self._remove_field_roles(ret_fields, set(six.iterkeys(self.field_mapping)),
                                                       FieldRole.FEATURE))
        target._fields = list(self._set_singleton_role(ret_fields, self.field_mapping))


class FieldContinuityOperation(DFAdapterOperation):
    ...
    def execute(self, sources, target):
        ...
        target._fields = fields[0]


class FieldKVConfigOperation(DFAdapterOperation):
...
class StaticFieldChangeOperation(DFAdapterOperation):
    ...
    def execute(self, sources, target):
        ...
        dup_fields = [MLField.copy(f) for f in self.fields]
        if self.is_append:
            src = list([copy.deepcopy(f) for f in existing_fields[0]])
            src.extend(dup_fields)
            target._fields = src
        else:
            target._fields = self.fields


class ProgrammaticFieldChangeOperation(DFAdapterOperation):
    ...
    def execute(self, sources, target):
        ...
        dup_fields = [MLField.copy(f, None) for f in fields]
        if self.is_append:
            src = list([copy.deepcopy(f) for f in existing_fields[0]])
            src.extend(dup_fields)
            target._fields = src
...
class MergeFieldsOperation(DFAdapterOperation):
    ...
    def execute(self, sources, target):
        ...
        def trans_columns(tid, fields):
            ...
            for field in fields:
                ...
                if exc_cols and field.name in exc_cols:
                    continue
                new_col = MLField.copy(field, field.role)
        ...
        target._fields = list(reduce(lambda fa, fb: fa + fb, new_fields, []))
        target._uplink = sources
...
class SetPartitionOperation(DFAdapterOperation):
    def __init__(self, ds_partitions):
        self._partitions = ds_partitions

    def execute(self, sources, target):
        fields = self._get_fields_list_from_eps(sources)
        target._partitions = self._partitions
        target._fields = copy.deepcopy(fields[0])
