...
require(["nbextensions/widgets/widgets/js/widget", "nbextensions/widgets/widgets/js/manager", "jquery"], function(widget, manager, $) {
    ...
    var HTMLNotifier = widget.DOMWidgetView.extend({
        ...
        _handle_route_msg: function(msg) {
            ...
            pyodps.notify_config = $.parseJSON(msg);
            pyodps.notify_config.icon = IMAGE_DATA;
            if (pyodps.restart_handler === undefined) {
                pyodps.restart_handler = function() {
                    pyodps.last_notify_prompt = -1;
                };
                $([IPython.events]).on('kernel_restarting.Kernel', pyodps.restart_handler);
            }
            ...
            var notifier = function() {
                ...
                if (cell_element.hasClass('running'))
                    ...
                    if (prompt_num !== pyodps.last_notify_prompt) {
                        pyodps.last_notify_prompt = prompt_num;
                        new Notification('PyODPS', pyodps.notify_config);
                    }
                }
            };
            window.setTimeout(notifier, 100);
    ...
    manager.WidgetManager.register_widget_view('HTMLNotifier', HTMLNotifier);
