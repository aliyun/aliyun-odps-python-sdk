...
from .core import tn, in_coverage_mode
...
def _get_odps_json(odps):
    return json.dumps(dict(access_id=odps.account.access_id,
...
def test_drop(odps):
    ...
    tempobj.register_temp_volume_partition(odps, ('non_exist_vol', 'non_exist_vol_part'))
    tempobj.clean_stored_objects(odps)


def test_cleanup(odps):
    odps.execute_sql('drop table if exists {0}'.format(TEMP_TABLE_NAME))
    odps.execute_sql('create table {0} (col1 string) lifecycle 1'.format(TEMP_TABLE_NAME))
...
def test_plenty_create(odps):
    del_insts = [odps.run_sql('drop table {0}'.format(tn('tmp_pyodps_create_temp_%d' % n))) for n in range(10)]
    ...
    sleep(5)
    trial = 4
    case = lambda: all(not odps.exist_table(tn('tmp_pyodps_create_temp_%d' % tid)) for tid in range(10))
    while not case():
        trial -= 1
        sleep(5)
        if trial == 0:
