...
class Test(TestBase):
    ...
    def testMapReduce(self):
        ...
        expr = self.expr.map_reduce(mapper._func, mapper_output_types=mapper.output_types)
        self.assertEqual(len(expr.schema.columns), len(mapper.output_types))
        self.assertListEqual(expr.schema.types,
                             [types.validate_data_type(tps) for tps in mapper.output_types])
