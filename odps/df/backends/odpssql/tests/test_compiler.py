...
class Test(TestBase):
    ...
    def testElementCompilation(self):
        ...
        data = ['2.3', '3.3']
        self._testify_udf([str(Decimal(d) + 1) for d in data], [(d, ) for d in data])
    ...
    def testArithmeticCompilation(self):
        ...
        self.engine.compile((self.expr.id // self.expr.id).rename('id'))
        self._testify_udf([l // l for l, r in data], [d for d in [(30, 30)]])
    ...
    def testStringCompilation(self):
        ...
        data = ['ab' * 15, 'def']
        self._testify_udf([d[0: 10: 2] for d in data], [(d, 0, 10, 2) for d in data])

        self.engine.compile(self.expr.name.slice(-5, -1))
        data = ['ab' * 15, 'def']
        self._testify_udf([d[-5: -1] for d in data], [(d, -5, -1) for d in data])

        self.engine.compile(self.expr.name.slice(-5, step=1).value_counts(sort=False))
        data = ['ab' * 15, 'def']
        self._testify_udf([d[-5:] for d in data], [(d, -5, 1) for d in data])
    ...
    def testReductionCompilation(self):
        ...
        self.engine.compile(expr)
        self._testify_udf([9, ], [[r, ] for r in data])
        ...
        self.engine.compile(expr)
        self._testify_udf(['9.6', ], [[str(r), ] for r in data])
    ...
    def testJoin(self):
        ...
        self.assertEqual(to_str(expected),
                         to_str(self.engine.compile(joined[lambda x, y: y.name.rename('new_name'), e.id],
                                                    prettify=False)))

        joined = e.join(e1, ['fid', 'id'])

        joined = joined[e1.name, e.id]
        expected = 'SELECT t3.`name`, t3.`id` AS `id_x`, t3.`fid`, t3.`isMale`, t3.`scale`, ' \
                   't3.`birth`, t5.`name_y`, t5.`id` AS `id_y` \n' \
        ...
        expected = 'SELECT t7.`name` \n' \
                   ...
                   'INNER JOIN \n' \
                   '  (\n' \
                   '    SELECT t1.`name`, t1.`fid` AS `fid_x`, t3.`id` AS `id_y` \n' \
                   '    FROM mocked_project.`pyodps_test_expr_table` t1 \n' \
                   '    INNER JOIN \n' \
                   '      mocked_project.`pyodps_test_expr_table2` t3\n' \
                   '    ON t1.`name` == t3.`name`\n' \
                   '  ) t7\n' \
                   'ON t6.`name_x` == t7.`name`'
        ...
        expected = 'SELECT t9.`fid`, t9.`id`, t2.`name`, ' \
                   't2.`isMale`, t2.`scale`, t2.`birth` \n' \
                   'FROM (\n' \
        ...
        expected = "SELECT * \n" \
                   ...
                   "  ) t10\n" \
                   "ON t1.`name` == (CONCAT('tt', t10.`name2`))"
        self.assertEqual(to_str(expected), to_str(self.engine.compile(joined, prettify=False)))
    ...
    def testJoinMultipleFilter(self):
        ...
        expected = 'SELECT * \n' \
                   ...
                   ') t4 \n' \
                   'WHERE t4.`name2` IS NULL'
        self.assertEqual(to_str(expected), to_str(self.engine.compile(expr)))
    ...
    def testSelfJoin(self):
        ...
        expected = 'SELECT t3.`name`, t3.`id` AS `id_x`, t4.`id` AS `id_y`, t5.`id` \n' \
                   'FROM (\n' \
    ...
    def testLeftJoin(self):
        ...
        expected = 'SELECT t4.`name` AS `name_x`, t4.`id` AS `id_x`, t4.`fid` AS `fid_x`, ' \
                   't4.`isMale` AS `isMale_x`, t4.`scale` AS `scale_x` \n' \
                   'FROM (\n' \
                   '  SELECT t1.`name`, t1.`id`, t1.`fid`, t1.`isMale`, t1.`scale` \n' \
                   '  FROM mocked_project.`pyodps_test_expr_table` t1\n' \
                   ') t4 \n' \
    ...
    def testMapJoin(self):
        ...
        expected = 'SELECT /*+mapjoin(t6)*/ t5.`name` AS `name_x`, t5.`id` AS `id_x`, t6.`name` AS `name_y`, ' \
                   't6.`id` AS `id_y`, t6.`fid`, t6.`isMale`, t6.`scale`, t6.`birth` \n' \
                   'FROM (\n' \
                   '  SELECT t1.`name`, t1.`id` \n' \
                   '  FROM mocked_project.`pyodps_test_expr_table` t1\n' \
                   ') t5 \n' \
