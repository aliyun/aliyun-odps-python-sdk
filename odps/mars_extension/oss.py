...
def _clean_oss_object(path, access_id=None, secret_access_key=None, endpoint=None, bucket_name=None):
...
def _copy_to_ext_table(expr, odps, dim_cols, val_col, chunks, partitions=None,
                       oss_path=None, handler=None, hints=None, **oss_opts):
    column_names = list(dim_cols) + [val_col]
    column_types = [df_type_to_odps_type(expr.schema[n].type) for n in column_names]

    if partitions is not None:
        partitions = [partitions] if not isinstance(partitions, (list, tuple, set)) else partitions
        partition_types = ['string'] * len(partitions)
        target_schema = Schema.from_lists(column_names, column_types, partitions, partition_types)
    ...
    ext_table = odps.create_table(
        ...
        location='oss://%s:%s@%s/%s/%s/data' % (
            oss_opts.get('access_id'), oss_opts.get('secret_access_key'),
            oss_opts.get('endpoint').split('://')[1],
            oss_opts.get('bucket_name'), oss_path
        ),
        resources=handler,
        lifecycle=14,
    )
    ...
    if partitions:
        shuffled = expr.select(column_names).reshuffle(partitions, sort=partitions + dim_cols)
    else:
    ...
    shuffled.persist(ext_table.name, overwrite=False, partitions=partitions, hints=odps_hints)
...
def _get_shapes(expr, dim_columns, partitions=None, hints=None):
    if partitions is None:
        shapes = expr[list((expr[col].max() + 1).rename(col) for col in dim_columns)].execute(hints=hints)
        return tuple(_as_scalar(s) for s in shapes[0])
    else:
        odps_hints = {'odps.task.merge.enabled': False}
        if hints:
            odps_hints.update(odps_hints)
        shapes = expr.groupby(partitions).agg(
            list((expr[col].max() + 1).rename(col) for col in dim_columns)).execute(
            hints=odps_hints)
        return dict((','.join(tuple(record[partitions])),
                     tuple(_as_scalar(s) for s in record[dim_columns])) for record in shapes)


def _preprocess(expr, table_name, columns, partitions=None, hints=None):
    if partitions:
        expr_grouped = expr.groupby(partitions)
        expr_ranked = expr[[(expr_grouped[col].dense_rank() - 1).rename(col + '_continuous') for col in columns]]
    else:
        expr_ranked = expr[[(expr.groupby(Scalar(1))[col].dense_rank() - 1).rename(
            col + '_continuous') for col in columns]]
...
def _get_df_partitions(df, partitions):
    if df._source_data is not None and isinstance(df._source_data, Table) and \
            set([p.name for p in df._source_data.schema.partitions]) == set(partitions):
...
def to_mars_tensor_via_oss(expr, dim_columns, value_column, chunks=None, chunk_size=None,
                           oss_access_id=None, oss_access_key=None,
                           oss_endpoint=None, oss_bucket_name=None, oss_path=None, odps=None,
                           discontinuous_columns=None, new_table_name=None, partitions=None, shape=None,
                           sparse=False, oss_file_exist=False, handler='public/resources/chunkwriter.jar',
                           hints=None):
    if (discontinuous_columns is not None) != (new_table_name is not None):
        raise ValueError('if some dimension columns are discontinuous,'
    ...
    if discontinuous_columns is not None:
        if not odps.exist_table(new_table_name):
            _preprocess(expr, new_table_name, discontinuous_columns, partitions, hints=hints)
        expr = odps.get_table(new_table_name).to_df()
        disordered_cols = discontinuous_columns or []
        dim_columns = [col + '_continuous' if col in disordered_cols else col for col in dim_columns]
    oss_opts = dict(endpoint=oss_endpoint, bucket_name=oss_bucket_name,
                    access_id=oss_access_id, secret_access_key=oss_access_key)
    ...
    if chunks is not None:
        warnings.warn('chunks is deprecated, use chunk_size instead', DeprecationWarning)
    ...
    if not oss_file_exist:
        _copy_to_ext_table(expr, odps, dim_columns, value_column, chunk_size,
                           oss_path=oss_path, partitions=partitions, handler=handler, hints=hints, **oss_opts)

    if partitions is None:
        # for non-partitioned table
        path = 'oss://%s/data/*.parquet' % oss_path
        tensor = read_coo(path, dim_columns, value_column, shape=shape,
                          chunk_size=chunk_size, dtype=dtype, sparse=sparse, **oss_opts)
        return tensor
    else:
        ...
        for partition in partition_values:
            ...
            partition_key = ','.join(partition[p] for p in partitions)
            tensor = read_coo(path, dim_columns, value_column, shape=shape[partition_key],
                              chunk_size=chunk_size, dtype=dtype, sparse=sparse, **oss_opts)
...
def persist_tensor_via_oss(odps, *args, **kwargs):
    ...
    if isinstance(tensor, dict):
        for p, t in tensor.items():
            ...
            writer_tensor = write_coo(t, data_path, dim_columns, value_column,
                                      global_index=True, **oss_opts)
        ...
        writer_tensor = write_coo(tensor, data_path, dim_columns, value_column,
                                  global_index=True, **oss_opts)
    ...
    ext_column_names = list(chain(*([c, 'global_' + c] for c in dim_columns))) + [value_column]
    if partitions:
        if isinstance(partitions, six.string_types):
            partitions = [partitions]
        target_schema = Schema.from_lists(column_names, column_types,
                                          partitions, ['string'] * len(partitions))
    ...
    fields = [ext_df['global_' + f].rename(f) for f in target_schema.names[:-1]] + target_schema.names[-1:]
    if partitions:
