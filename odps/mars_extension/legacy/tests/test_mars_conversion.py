...
class Test(TestBase):
    ...
    def testNoPartitionToMars(self):
        ...
        df = DataFrame(pdf).persist(tn('test_no_partition_dense_to_mars'), lifecycle=1, odps=self.odps)

        oss_access_id, oss_secret_access_key, oss_bucket_name, oss_endpoint = self.config.oss

        # test dense
        t = df.to_mars_tensor_via_oss(['i', 'j'], 'v', 15,
                                      oss_access_id=oss_access_id,
                                      oss_access_key=oss_secret_access_key,
                                      oss_bucket_name=oss_bucket_name,
                                      oss_endpoint=oss_endpoint,
        ...
        self.odps.delete_table(table_name, if_exists=True)
        self.odps.persist_tensor_via_oss(t, table_name, ['x', 'y'], 'z',
                                         oss_access_id=oss_access_id,
                                         oss_access_key=oss_secret_access_key,
                                         oss_bucket_name=oss_bucket_name,
                                         oss_endpoint=oss_endpoint,
        ...
        with self.odps.get_table(table_name).open_reader() as reader:
            result = sorted([(r['x'], r['y'], r['z']) for r in reader], key=lambda x: (x[0], x[1]))
            self.assertEqual(kv, result)

        # test dense without setting chunks
        t = df.to_mars_tensor_via_oss(['i', 'j'], 'v',
                                      oss_access_id=oss_access_id,
                                      oss_access_key=oss_secret_access_key,
                                      oss_bucket_name=oss_bucket_name,
                                      oss_endpoint=oss_endpoint,
    ...
    def testPartitionsToMars(self):
        ...
        df = self.odps.create_table(source_table_name,
                                    ('i int, j int, v double', 'g string'), if_not_exists=True).to_df()
        ...
        for g in (1, 2):
            ...
            kvs.append(kv)
            pdf = pd.DataFrame(kv, columns=['i', 'j', 'v'])
            DataFrame(pdf).persist(source_table_name, lifecycle=1,
                                   partition='g=%s' % g, odps=self.odps)

        oss_access_id, oss_secret_access_key, oss_bucket_name, oss_endpoint = self.config.oss

        # test dense
        td = df.to_mars_tensor_via_oss(['i', 'j'], 'v', 15,
                                       oss_access_id=oss_access_id,
                                       oss_access_key=oss_secret_access_key,
                                       oss_bucket_name=oss_bucket_name,
                                       oss_endpoint=oss_endpoint,
        ...
        self.odps.persist_tensor_via_oss(td, out_table_name, ['x', 'y'], 'z',
                                         oss_access_id=oss_access_id,
                                         oss_access_key=oss_secret_access_key,
                                         oss_bucket_name=oss_bucket_name,
                                         oss_endpoint=oss_endpoint,
        ...
        for partition, kv in zip(td.keys(), kvs):
            with self.odps.get_table(out_table_name).get_partition(partition).open_reader() as reader:
                result = sorted([(r['x'], r['y'], r['z']) for r in reader], key=lambda x: (x[0], x[1]))
                self.assertEqual(kv, result)
    ...
    def testPersistToTensor(self):
        ...
        t_from_oss = df.to_mars_tensor_via_oss(['x', 'y'], 'z', 6,
                                               oss_access_id=oss_access_id,
                                               oss_access_key=oss_secret_access_key,
                                               oss_bucket_name=oss_bucket_name,
                                               oss_endpoint=oss_endpoint,
    ...
    def testPartitionsPersistToTensor(self):
        ...
        self.odps.delete_table(table_name, if_exists=True)
        self.odps.persist_tensor_via_oss({'g=1': t1, 'g=2': t2}, table_name, ['x', 'y'], 'z',
                                         oss_access_id=oss_access_id,
                                         oss_access_key=oss_secret_access_key,
                                         oss_bucket_name=oss_bucket_name,
                                         oss_endpoint=oss_endpoint,
