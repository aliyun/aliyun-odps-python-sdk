...
try:
    import mars
    from odps.mars_extension.legacy.core import create_mars_cluster, \
        to_mars_dataframe, persist_mars_dataframe
except ImportError:
    mars = None
...
class Test(TestBase):
    ...
    def _create_table(self, table_name):
        ...
        return self.odps.create_table(
            table_name, schema=Schema.from_lists(fields, types), stored_as='aliorc', lifecycle=1)
    ...
    def testCreateMarsCluster(self):
        ...
        try:
            ...
            self.odps.persist_mars_dataframe(
                df_head, mars_des_table_name, unknown_as_string=True,
    ...
    def testMarsDataFrame(self):
        ...
        try:
            self.assertTrue(client._with_notebook)
            status_code = requests.get(client.notebook_endpoint).status_code
            self.assertEqual(status_code, 200)

            mars_source_table_name = tn('mars_df')

            data = pd.DataFrame({'c1': np.random.rand(100),
                                 'c2': np.random.randint(0, 100, (100,))})
            self.odps.delete_table(mars_source_table_name, if_exists=True)
            self.odps.create_table(mars_source_table_name, 'c1 double, c2 int',
                                   stored_as='aliorc', lifecycle=1)
    ...
    def testRunScript(self):
        ...
        try:
            ...
            code = BytesIO(to_binary(script.format(
                mars_source_table_name, self.odps.endpoint, mars_des_table_name, self.odps.endpoint
            )))
    ...
    def testRunMarsJob(self):
        ...
        def func(s_name, d_name):
            ...
            df = odps_entry.to_mars_dataframe(s_name, runtime_endpoint=odps_entry.endpoint).to_pandas()
            odps_entry.persist_mars_dataframe(df, d_name, unknown_as_string=True,
                                              runtime_endpoint=odps_entry.endpoint)

        self.odps.run_mars_job(
            func, args=(mars_source_table_name, mars_des_table_name), name=str(uuid.uuid4()),
    ...
    def testViewTable(self):
        ...
        client = self.odps.create_mars_cluster(1, 4, 8, name=str(uuid.uuid4()))
        try:
            df = self.odps.to_mars_dataframe(mars_view_table_name,
                                             runtime_endpoint=self.odps.endpoint)
            result = df.execute().to_pandas()
            expected = pd.DataFrame({'view_col1': [1, 2], 'view_col2': ['test1', 'test2']})
            pd.testing.assert_frame_equal(result, expected)
        finally:
            client.stop_server()
    ...
    def testPersistOdps2Types(self):
        ...
        table = self.odps.create_table(mars_source_table_name,
                                       ...
                                       'col6 datetime,'
                                       'col7 date',
                                       lifecycle=1, stored_as='aliorc')

        contents = [
            [0, 1, 2, 1.0, pd.Timestamp('1998-02-15 23:59:21.943829154'), datetime.today(), date.today()],
            [0, 1, 2, 1.0, pd.Timestamp('1998-02-15 23:59:21.943829154'), datetime.today(), date.today()],
            [0, 1, 2, 1.0, pd.Timestamp('1998-02-15 23:59:21.943829154'), datetime.today(), date.today()],
        ]
        ...
        try:
            ...
            def func(d_name):
                ...
                df = pd.DataFrame(contents, columns=['col' + str(i + 1) for i in range(7)])
                odps_entry.persist_mars_dataframe(df, d_name, unknown_as_string=True,
                                                  runtime_endpoint=odps_entry.endpoint)
    ...
    def testArrowTunnel(self):
        ...
        r = self.odps.to_mars_dataframe(mars_source_table_name).execute().to_pandas()
        expected = self.odps.get_table(mars_source_table_name).to_df().to_pandas()
        pd.testing.assert_frame_equal(r, expected)

        data = pd.DataFrame({'col1': np.random.rand(1000,),
                             'col2': np.random.randint(0, 100, (1000,)),
                             'col3': np.random.choice(['a', 'b', 'c'], size=(1000,))})

        df = md.DataFrame(data, chunk_size=300)
        self.odps.persist_mars_dataframe(df, mars_des_table_name, unknown_as_string=True)
        expected = self.odps.get_table(mars_des_table_name).to_df().to_pandas()
        pd.testing.assert_frame_equal(expected.sort_values('col1').reset_index(drop=True),
    ...
    def testArrowTunnelSinglePart(self):
        ...
        df = md.DataFrame(data, chunk_size=300)
        self.odps.persist_mars_dataframe(df, mars_des_table_name, partition='pt=test_part',
    ...
    def testArrowTunnelMultipleParts(self):
        ...
        r = self.odps.to_mars_dataframe(
            mars_source_table_name, append_partitions=True, add_offset=True).execute().to_pandas()
        expected = table.to_df().to_pandas()
        pd.testing.assert_frame_equal(r, expected)

        r = self.odps.to_mars_dataframe(
            mars_source_table_name, partition='pt>test_part1', append_partitions=True,
            add_offset=True).execute().to_pandas()
        expected = table.to_df().to_pandas().query('pt>"test_part1"').reset_index(drop=True)
        pd.testing.assert_frame_equal(r, expected)
    ...
    def testExistedPartition(self):
        ...
        self.odps.persist_mars_dataframe(df, mars_source_table_name,
                                         partition='pt=test', unknown_as_string=True)
