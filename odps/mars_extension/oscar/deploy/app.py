...
from mars.deploy.kubernetes.config import VolumeConfig, \
    MarsReplicationConfig, MarsSupervisorsConfig, MarsWorkersConfig
...
class CupidMarsConfigMixin:
    ...
    def get_local_app_module(mod_name):
        ...
        return supervisor.__name__.rsplit('.', 1)[0] + '.' + mod_name
...
class CupidMarsSupervisorsConfig(CupidMarsConfigMixin, MarsSupervisorsConfig):
    def __init__(self, *args, **kwargs):
        if os.environ.get('VM_ENGINE_TYPE') == 'hyper':
            default_port = UI_PORT
        else:
            default_port = None
        kwargs['web_port'] = kwargs.get('web_port', None) or default_port
        super().__init__(*args, **kwargs)

    def add_default_envs(self):
        super().add_default_envs()
        self.remove_env('MARS_USE_CGROUP_STAT')
        self.add_env('MARS_POD_ROLE', 'supervisor')
...
class CupidMarsNotebooksConfig(CupidMarsConfigMixin, MarsReplicationConfig):
...
class CupidGSCoordinatorConfig(CupidMarsConfigMixin, MarsReplicationConfig):
...
class CupidKubernetesCluster(KubernetesCluster):
    _supervisor_config_cls = CupidMarsSupervisorsConfig
    _worker_config_cls = CupidMarsWorkersConfig
    _default_service_port = None
    _default_web_port = None
...
class MarsCupidServer(object):
    ...
    def _main(self, argv=None):
        ...
        supervisor_extra_env = args_dict.get('supervisor_extra_env') or dict()
        if args_dict.get('instance_idle_timeout') is not None:
            idle_timeout = args_dict.pop('instance_idle_timeout')
            supervisor_extra_env['MARS_INSTANCE_IDLE_TIMEOUT'] = str(idle_timeout)
        args_dict['supervisor_extra_env'] = supervisor_extra_env
