...
from mars.services.cluster.backends import AbstractClusterBackend, \
...
class CupidK8SClusterBackend(_ClusterBackend):
    ...
    async def _extract_pod_name_ep(self, pod_data):
        ...
        if self._pod_to_port.get(pod_name):
            ...
            try:
                pod_kv_data = await asyncio.to_thread(self.cupid_client.get_kv, pod_name)
            except OSError:
...
class CupidCommandRunnerMixin:
    ...
    async def register_application(self, app_name, endpoint):
        await asyncio.to_thread(
            self._cupid_context.register_application, app_name, endpoint)
    ...
    async def write_node_endpoint(self):
        ...
        await asyncio.to_thread(cupid_client.put_key,
                                os.environ['MARS_K8S_POD_NAME'], content)
