...
try:
    import mars
    from odps.mars_extension.oscar.core import to_mars_dataframe, persist_mars_dataframe
except ImportError:
    mars = None
...
class Test(TestBase):
    ...
    def testArrowTunnel(self):
        ...
        r = self.odps.to_mars_dataframe(mars_des_table_name, chunk_size=200).execute().to_pandas()
        expected = self.odps.get_table(mars_des_table_name).to_df().to_pandas()
        pd.testing.assert_frame_equal(r.reset_index(drop=True), expected.reset_index(drop=True))
    ...
    def testArrowTunnelMultipleParts(self):
        ...
        r = self.odps.to_mars_dataframe(
            mars_source_table_name, partition='pt1>test_part1', append_partitions=True,
            add_offset=True).execute().to_pandas()
        expected = table.to_df().to_pandas().query('pt1>"test_part1"').reset_index(drop=True)
        pd.testing.assert_frame_equal(r, expected)

        r = self.odps.to_mars_dataframe(
            mars_source_table_name, partition='pt2=invalid & pt1>test_part1', append_partitions=True,
            ).execute().to_pandas()
