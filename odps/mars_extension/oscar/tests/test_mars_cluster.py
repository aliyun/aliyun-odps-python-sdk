...
try:
    import mars
    from odps.mars_extension.oscar.core import create_mars_cluster, \
...
class Test(TestBase):
    ...
    def testCreateMarsCluster(self):
        ...
        try:
            ...
            expected = self.odps.get_table(mars_source_table_name).to_df().to_pandas()['id'].head(2)
    ...
    def testMultipleParts(self):
        ...
        try:
            ...
            r = self.odps.to_mars_dataframe(
                mars_source_table_name, partition='pt=max_pt()', append_partitions=True,
                add_offset=True).execute().to_pandas()
            expected = table.to_df().to_pandas().query('pt=="test_part4"').reset_index(drop=True)
            pd.testing.assert_frame_equal(r, expected)
        finally:
            client.stop_server()
