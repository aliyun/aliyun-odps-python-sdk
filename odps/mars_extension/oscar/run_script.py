...
def run_script(script, data=None, n_workers=1, mode='exec', command_argv=None,
               retry_when_fail = False, odps_params=None, session=None, run_kwargs=None):
    ...
    return op(inputs).execute(session=session, **(run_kwargs or {})).fetch(session=session)
