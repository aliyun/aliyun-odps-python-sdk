...
from ..utils import use_odps2_type, pd_type_to_odps_type, \
    rewrite_partition_predicate, check_partition_exist
...
def create_mars_cluster(odps, worker_num=1, worker_cpu=8, worker_mem=32, cache_mem=None,
                        ...
                        with_notebook=False, notebook_cpu=None, notebook_mem=None,
                        with_graphscope=False, coordinator_cpu=None, coordinator_mem=None,
                        timeout=None, extra_modules=None, resources=None, instance_id=None,
                        name='default', if_exists='reuse', project=None, **kw):
    """
    ...
    kw = dict(worker_num=worker_num, worker_cpu=worker_cpu, worker_mem=worker_mem,
              ...
              supervisor_num=supervisor_num, supervisor_cpu=supervisor_cpu,
              supervisor_mem=supervisor_mem, with_notebook=with_notebook,
              notebook_cpu=notebook_cpu, notebook_mem=notebook_mem,
              with_graphscope=with_graphscope, coordinator_cpu=coordinator_cpu,
...
def to_mars_dataframe(odps, table_name, shape=None, partition=None, chunk_bytes=None,
                      sparse=False, columns=None, add_offset=None, calc_nrows=True,
                      incremental_index=False, use_arrow_dtype=False,
                      string_as_binary=None, chunk_size=None,
                      memory_scale=None, runtime_endpoint=None,
    ...
    :param incremental_index: ensure range index incremental
    ...
    return read_odps_table(odps.get_table(table_name), shape, partition=partition,
                           ...
                           incremental_index=incremental_index, chunk_size=chunk_size,
                           ...
                           with_split_meta_on_tile=with_split_meta_on_tile, extra_params=extra_params)
...
def run_script_in_mars(odps, script, mode='exec', n_workers=1, command_argv=None, **kw):
    ...
    runtime_endpoint = kw.pop('runtime_endpoint', None) or kw.pop('cupid_internal_endpoint', None)
...
def run_mars_job(odps, func, args=(), kwargs=None, retry_when_fail=False, n_output=None, **kw):
    ...
    try:
        ...
        r.op.extra_params['endpoint'] = runtime_endpoint or cupid_options.cupid.runtime.endpoint
        r.execute()
    finally:
        if task_name is None:
            client.stop_server()
