...
def _handle_write_table_data(sock):
    ...
    try:
        ...
        logger.debug('Start writing table block, block id: %s', writer_config['_block_id'])
        ...
        with block_writer.open_arrow_writer() as cupid_writer:
            ...
            while True:
                try:
                    ...
                    break
            arrow_writer.close()
        logger.debug('Write table block finished, block id: %s', writer_config['_block_id'])
        block_writer.commit()
    ...
    try:
        ...
        if not table.schema.partitions:
            ...
            if check_partition_exist(table, partition_desc):
                ...
                _write_request_result(sock, result=[str(pt.partition_spec) for pt in parts])
        else:
            _write_request_result(sock, result=[str(pt.partition_spec) for pt in table.partitions])
    except:
...
def _handle_create_table_download_session(sock):
    try:
        ...
        logger.debug('Start creating download session of table %s from cupid, columns %r',
                     table_name, columns)
        while True:
            try:
                download_session = cupid_session.create_download_session(
                    data_src, split_size=split_size, columns=columns,
...
def _handle_commit_table_upload_session(sock):
    try:
        ...
        upload_session = CupidTableUploadSession(
            ...
            handle=commit_config['cupid_handle'], blocks=commit_config['blocks'])
...
class CupidServiceClient:
    ...
    def write_table_data(self, writer_config, to_store_data, write_batch_size=None):
        ...
        while len(batch_data) > 0:
            batch = pa.RecordBatch.from_pandas(batch_data, schema=schema, preserve_index=False)
            arrow_writer.write_batch(batch)
            batch_idx += 1
            batch_data = to_store_data[batch_size * batch_idx: batch_size * (batch_idx + 1)]
        arrow_writer.close()
    ...
    def create_table_download_session(self, odps_params, table_name, partition, columns,
                                      worker_count=None, split_size=None, max_chunk_num=None,
        ...
        self._send_cupid_service_request(REQUEST_TYPE_CREATE_TABLE_DOWNLOAD_SESSION, cmd_pack)
    ...
    def create_table_upload_session(self, odps_params, table_name):
        ...
        self._send_cupid_service_request(REQUEST_TYPE_CREATE_TABLE_UPLOAD_SESSION, cmd_pack)
        result = self._fetch_cupid_service_result()
        return result['handle']

    def commit_table_upload_session(self, odps_params, table_name, cupid_handle, blocks,
                                    overwrite=True):
        cmd_pack = {
            'odps_params': odps_params,
            'table_name': table_name,
        ...
        self._send_cupid_service_request(REQUEST_TYPE_COMMIT_TABLE_UPLOAD_SESSION, cmd_pack)
