...
from mars.serialization.serializables import StringField, Int64Field, SeriesField, DictField, \
    BoolField, AnyField
...
from ....errors import ConnectTimeout
...
try:
    from mars.dataframe.datasource.core import IncrementalIndexDatasource, IncrementalIndexDataSourceMixin
    _BASE = (IncrementalIndexDatasource, ColumnPruneSupportedDataSourceMixin,
...
class DataFrameReadTable(*_BASE):
    ...
    def __init__(self, odps_params=None, table_name=None, partition_spec=None,
                 ...
                 incremental_index=True, use_arrow_dtype=None, string_as_binary=None,
                 memory_scale=None, append_partitions=None,
                 last_modified_time=None, with_split_meta_on_tile=False,
                 retry_times=None, **kw):
        ...
        try:
            ...
            for partition_spec in parts:
                splits, split_size = cupid_client.create_table_download_session(
                    op.odps_params, op.table_name, partition_spec, op.columns, worker_count,
                    split_size, MAX_CHUNK_NUM, op.with_split_meta_on_tile)

                logger.debug('%s table splits have been created.', str(len(splits)))

                meta_chunk_rows = [split.meta_row_count for split in splits]
                if np.isnan(out_shape[0]):
                    est_chunk_rows = meta_chunk_rows
                else:
                    sp_file_sizes = np.array([sp.split_file_end - sp.split_file_start for sp in splits])
                ...
                if len(splits) == 0:
                    ...
                    for idx, split in enumerate(splits):
                        chunk_op = DataFrameReadTableSplit(
                            ...
                            incremental_index=op.incremental_index, dtypes=out_dtypes, sparse=op.sparse,
                            ...
                            memory_scale=op.memory_scale, extra_params=op.extra_params)
    ...
    def _tile_tunnel(cls, op):
        ...
        if len(data_srcs) == 0:
            ...
            index_value = parse_index(pd.RangeIndex(0))
            columns_value = parse_index(out_dtypes.index, store_data=True)
            out_chunk = chunk_op.new_chunk(None, shape=(0, out_shape[1]), dtypes=op.dtypes,
                                           index_value=index_value, columns_value=columns_value,
                                           index=(index_start, 0))
            out_chunks.append(out_chunk)
        else:
            ...
            for data_src in data_srcs:
                ...
                for i in range(chunk_count):
                    ...
                    chunk_op = DataFrameReadTableSplit(
                        ...
                        odps_params=op.odps_params, columns=op.columns,
                        ...
                        retry_times=op.retry_times, extra_params=op.extra_params)
...
class DataFrameReadTableSplit(*_BASE):
    ...
    def __init__(self, cupid_handle=None, split_index=None, split_file_start=None,
                 ...
                 odps_params=None, columns=None, nrows=None, incremental_index=None,
                 dtypes=None, string_as_binary=None, split_size=None,
                 use_arrow_dtype=None, memory_scale=None, estimate_rows=None,
                 meta_raw_size=None, retry_times=None, append_partitions=None, sparse=None, **kw):
        super(DataFrameReadTableSplit, self).__init__(
            ...
            _nrows=nrows, _incremental_index=incremental_index,
            _estimate_rows=estimate_rows, _split_size=split_size,
            _dtypes=dtypes, _append_partitions=append_partitions, _sparse=sparse,
            _meta_raw_size=meta_raw_size, _memory_scale=memory_scale, _retry_times=retry_times,
    ...
    def _execute_in_cupid(cls, ctx, op):
        ...
        data = arrow_table_to_pandas_dataframe(pa_table, use_arrow_dtype=op.use_arrow_dtype)[:op.nrows]
...
def read_odps_table(table, shape, partition=None, sparse=False, chunk_bytes=None, chunk_size=None,
                    columns=None, odps_params=None, add_offset=None, use_arrow_dtype=False,
                    string_as_binary=None, memory_scale=None, append_partitions=False,
                    with_split_meta_on_tile=False, incremental_index=True, extra_params=None):
    import pandas as pd

    if add_offset is not None:
        warnings.warn('add_offset is deprecated, please use '
                      'incremental_index instead', DeprecationWarning)
        if add_offset in (True, False):
            incremental_index = add_offset
    ...
    op = DataFrameReadTable(odps_params=odps_params, table_name=table_name, partition_spec=partition,
                            dtypes=dtypes, sparse=sparse, incremental_index=incremental_index,
                            columns=columns, use_arrow_dtype=use_arrow_dtype,
                            ...
                            with_split_meta_on_tile=with_split_meta_on_tile, retry_times=retry_times,
