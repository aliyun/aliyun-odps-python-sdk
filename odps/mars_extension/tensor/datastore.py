...
from mars.serialize import ValueType, ListField, TupleField, StringField, KeyField, BoolField
...
def write_coo(a, path, dim_cols, value_col, **storage_opts):
    if a.ndim > len(dim_cols):
        raise ValueError("dim_cols must have the same size as or greater than tensor's ndim")

    global_index = storage_opts.pop('global_index', True)
    op = TensorStoreCOO(dtype=a.dtype, path=path, dim_cols=dim_cols, value_col=value_col,
                        storage_options=json.dumps(storage_opts), sparse=a.issparse(),
